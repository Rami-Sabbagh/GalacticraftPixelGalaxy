...
import com.ramilego4game.mods.PixelGalaxy.Earth.world.gen.dungeon.GCPixelEarthRoomBoss;
import com.ramilego4game.mods.PixelGalaxy.Earth.world.gen.dungeon.GCPixelEarthRoomChests;
import com.ramilego4game.mods.PixelGalaxy.Earth.world.gen.dungeon.GCPixelEarthRoomEmpty;
import com.ramilego4game.mods.PixelGalaxy.Earth.world.gen.dungeon.GCPixelEarthRoomSpawner;
import com.ramilego4game.mods.PixelGalaxy.Earth.world.gen.dungeon.GCPixelEarthRoomTreasure;

public class GCPixelEarthChunkProvider extends ChunkProviderGenerate
{
	final short topBlockID = (short) GalacticraftPixelEarth.pixelGrass.blockID;
	final byte topBlockMeta = 5;
	final short fillBlockID = (short) GalacticraftPixelEarth.pixelDirt.blockID;
	final byte fillBlockMeta = 3;
	final short lowerBlockID = (short) GalacticraftPixelEarth.pixelStone.blockID;
	...
	{
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomEmpty(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		...
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new GCPixelEarthRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.bossRooms.add(new GCPixelEarthRoomBoss(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.treasureRooms.add(new GCPixelEarthRoomTreasure(null, 0, 0, 0, ForgeDirection.UNKNOWN));
	}

	private BiomeGenBase[] biomesForGeneration = { GCPixelEarthBiomeGenBase.moonFlat };

	private final GCCoreMapGenBaseMeta caveGenerator = new GCPixelEarthGenCaves();
	...
	public GCPixelEarthChunkProvider(World par1World, long par2, boolean par4)
	{
	...
	public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
	{
		...
		for (int x = 0; x < GCPixelEarthChunkProvider.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < GCPixelEarthChunkProvider.CHUNK_SIZE_Z; z++)
			{
				...
				for (int y = 0; y < GCPixelEarthChunkProvider.CHUNK_SIZE_Y; y++)
				{
					if (y < GCPixelEarthChunkProvider.MID_HEIGHT + yDev)
					{
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < GCPixelEarthChunkProvider.CHUNK_SIZE_X; x++)
				{
					for (int z = 0; z < GCPixelEarthChunkProvider.CHUNK_SIZE_Z; z++)
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * GCPixelEarthChunkProvider.CHUNK_SIZE_X + x, cz * GCPixelEarthChunkProvider.CHUNK_SIZE_Z + z) / GCPixelEarthChunkProvider.CRATER_PROB)
						{
