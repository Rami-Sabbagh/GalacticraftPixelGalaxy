package com.ramilego4game.mods.PixelGalaxy.Earth.world.gen.dungeon;
...
public class GCPixelEarthRoomSpawner extends GCCoreDungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public GCPixelEarthRoomSpawner(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	{
		return new GCPixelEarthRoomSpawner(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		for (final ChunkCoordinates spawnerCoords : this.spawners)
		{
			if (this.worldObj.getBlockId(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Block.mobSpawner.blockID)
			{
				...
				if (spawner != null)
				{
					spawner.getSpawnerLogic().setMobID(GCPixelEarthRoomSpawner.getMob(rand));
				}
			}
		}
	}
