...
public class WorldGenPixeTallGrass extends WorldGenerator {
	
	private Block block;
	private Block spreadBlock;
	private int spreadBlockMeta;

	public WorldGenPixeTallGrass(Block block, Block spreadBlock, int spreadBlockMeta) {
		this.block = block;
	...
	public boolean generate(World world, Random rand, int x, int y, int z) {
		...
		for (int j = 0; j < 16; j++) {
			for (k = 0; k < 16; k++) {
				for (m = 4; m < 8; m++) {
					if (arrayOfBoolean[(j * 16 + k) * 8 + m] && world.getBlock(x + j, y + m + 1, z + k) == PixelGalaxyBlocks.pixelDirt && world.getSavedLightValue(EnumSkyBlock.Sky, x + j, y + m, z + k) > 0) {
						world.setBlock(x + j, y + m + 1, z + k, PixelGalaxyBlocks.greenGrass, 0, 2);
					}
				}
			}
		}
